...
class FewShotSeg(nn.Module):
    ...
    def forward(self, supp_imgs, fore_mask, back_mask, qry_imgs, saliency_pred):
        ...
        for epi in range(batch_size):
            ...
            part = 3
            supp_fg_fts = [[self.getFeatures_new(supp_fts[way, shot, [epi]],
                                             fore_mask[way, shot, [epi]], part)
                            for shot in range(n_shots)] for way in range(n_ways)]
            supp_bg_fts = [[self.getFeatures_new(supp_fts[way, shot, [epi]],
                                             back_mask[way, shot, [epi]], part)
                            for shot in range(n_shots)] for way in range(n_ways)]
            fg_prototypes, bg_prototypes = self.getPrototype_new(supp_fg_fts, supp_bg_fts)

            if not self.training:
              fg_prototypes, bg_prototypes = self.kmeans_adjustment(epi, n_shots, n_ways, supp_fts, fore_mask, back_mask, fg_prototypes, bg_prototypes, part)
            ...
            if self.config['align'] and self.training:
                align_loss_epi = self.alignLoss_new(qry_fts[:, epi], pred, supp_fts[:, :, epi],
    ...
    def kmeans_adjustment(self, epi, n_shots, n_ways, supp_fts, fore_mask, back_mask, fg_prototypes, bg_prototypes, part):
            ...
            for way in range(n_ways):
                ...
                for ptp in ptps:
                    ...
                    for shot in range(n_shots):
                        new_fg_ptp.append(self.kmeans_prototypes(ptp, supp_fts[way, shot, [epi]], fore_mask[way, shot, [epi]]))
        ...
        for ptp in bg_prototypes:
            ...
            for way in range(n_ways):
                for shot in range(n_shots):
                    new_bg_ptp.append(self.kmeans_prototypes(ptp, supp_fts[way, shot, [epi]], back_mask[way, shot, [epi]]))
    ...
    def kmeans_prototypes(self, prototype, fts, mask):
        ...
        dist = F.cosine_similarity(fts * mask[None, ...], prototype[..., None, None], dim=1)
        weighted_prototype = torch.sum(dist * fts, dim=(2,3)) / (torch.sum(dist) + 1e-5) # (mask[None, ...].sum(dim=(2,3)) + 1e-5)
        ...
        for i in range(part):
          for j in range(part):
            if i == part - 1 and j == part - 1:
              ...
              mask_p = mask[:, i*squared_h:(i+1)*squared_h, j*squared_h:]
              ft_p = fts[:, :, i*squared_h:(i+1)*squared_h, j*squared_h:]
            elif i == part - 1:
              mask_p = mask[:, i*squared_h:, j*squared_h:(j+1)*squared_h]
              ft_p = fts[:, :, i*squared_h:, j*squared_h:(j+1)*squared_h]
            else:
              mask_p = mask[:, i*squared_h:(i+1)*squared_h, j*squared_h:(j+1)*squared_h]
              ft_p = fts[:, :, i*squared_h:(i+1)*squared_h, j*squared_h:(j+1)*squared_h]
            m_ft = torch.sum(ft_p * mask_p[None, ...], dim=(2,3)) \
              / (mask_p[None, ...].sum(dim=(2,3)) + 1e-5) # 1 x C
